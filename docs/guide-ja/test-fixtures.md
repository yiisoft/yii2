...
Yii のフィクスチャフレームワークにおける鍵となる概念は、いわゆる *フィクスチャ・オブジェクト* です。
...
ここで `FixturePath` はフィクスチャクラス・ファイルを含むディレクトリを意味し、`TableName` はフィクスチャと関連付けられているテーブルの名前です。
...
そうすると、テストメソッドの中でエイリアスを使ってフィクスチャ・オブジェクトにアクセスすることが出来るようになります。
例えば、
...
デフォルトでは、フィクスチャクラスは対応するデータファイルを探すときに、フィクスチャのクラス・ファイルを含むフォルダのサブフォルダである `data` フォルダの中を見ます。
...
同様な階層は、フィクスチャクラス・ファイルを編成するのにも使うことが出来ます。
