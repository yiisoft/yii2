...
* [[yii\base\Application::$sourceLanguage|ソース言語]] : ソース・コード中のテキストメッセージが書かれている言語。
...
> いくつかのメッセージソースを使っていて、それらが同じ方法で欠落した翻訳を取り扱うようにしたい場合は、対応するイベント・ハンドラを全てのメッセージソースそれぞれに割り当てなければなりません。
...
詳細は [データフォーマッタ](output-formatting.md) のセクションを参照してください。
