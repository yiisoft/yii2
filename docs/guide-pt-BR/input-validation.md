...
Como regra geral, você nunca deve confiar nos dados recebidos dos usuários finais, e deve sempre validá-lo antes de colocá-lo em uso.

Dado um [model (modelo)](structure-models.md) preenchido com entradas do usuário, você pode validar essas entradas chamando o método [[yii\base\Model::validate()]]. Esse método irá retornar um valor booleano que indica se a validação foi bem-sucedida ou não. Se não, você pode receber as mensagens de erro da propriedade [[yii\base\Model::errors]]. Por exemplo,
...
## Declarando Regras <span id="declaring-rules"></span>

Para fazer o `validate()` realmente funcionar, você deve declarar regras de validação para os atributos que você pretende validar. Isto deve ser feito sobrepondo o método [[yii\base\Model::rules()]]. O exemplo a seguir mostra como as regras de validação para o model (modelo) `ContactForm` são declarados:
...
```
O método [[yii\base\Model::rules()|rules()]] deve retornar um array de regras, cada um sendo é um array (matriz) no seguinte formato:
...
Para cada regra você precisa especificar, qual atributo a regra se aplica e qual é o tipo de regra. Você pode especificar o tipo de regra em uma das seguintes formas:

* O alias de um validador, tal como `required`, `in`, `date`, etc. Por favor, consulte os [Core Validators](tutorial-core-validators.md) para a lista  completa de validadores do núcleo.
* O nome de um método de validação na classe do model (modelo), ou uma função anônima. Por favor, consulte a subsecção [Inline Validators](#inline-validators) para mais detalhes.
* Um nome de classe qualificado. Por favor, consulte a subsecção [Standalone Validators](#standalone-validators) para mais detalhes.

Uma regra pode ser usado para validar um ou vários atributos, e um atributo pode ser validado por uma ou várias regras. A regra pode ser aplicada em determinados [cenários](structure-models.md#scenarios), especificando a opção `on`. Se você não especificar uma opção `on`, isso significa que a regra será aplicada a todos os cenários.

Quando o método `validate()` é chamado, ele segue as seguintes etapas para executar a validação:
