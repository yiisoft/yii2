...
Так как этот класс является поведением, то компоненту, к которому он прикреплён, будут также доступны свойства `prop1`
...
Прикрепить поведение к компоненту можно так же через [конфигурацию](concept-configurations.md), как показано ниже:
...
В заключение, давайте посмотрим на [[yii\behaviors\TimestampBehavior]] — поведение, которое позволяет автоматически
