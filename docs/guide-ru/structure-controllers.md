...
Вы можете сконфигурировать [[yii\base\Application::controllerMap|карту контроллеров]] в [настройках приложения](structure-applications.md#Конфигурации-приложения-)
следующим образом:
...
Когда в запросе не указан [маршрут](#Маршруты-), тогда будет использован маршрут указанный в данном свойстве.
...
Вы можете изменить контроллер по умолчанию следующим образом в [настройках приложения](structure-applications.md#Конфигурации-приложения-):
...
Когда [маршрут](#Маршруты-) содержит только ID контроллера, то подразумевается, что действие контроллера по умолчанию
было запрошено.
...
запрошенном [маршруте](#Маршруты-). Для выполнения запроса, контроллер пройдет через следующие этапы
жизненного цикла:
