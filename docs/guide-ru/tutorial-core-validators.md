...
- `captchaAction`: [маршрут](structure-controllers.md#Маршруты-), соответствующий
...
> Info: Как определить, является значение пустым или нет, более подробно описано в отдельной статье
  в секции [Пустые значения](input-validation.md#Обработка-пустых-входных-данных-).
...
валидатору [number](#yiivalidatorsnumbervalidatorinteger-). (Прим. пер.: корректным float числом).
...
> Трюк: Если вы хотите удалить пробелы вокруг значений атрибута, вы можете использовать валидатор [trim](#yiivalidatorsfiltervalidatortrim-).
...
Этот валидатор проверяет, что входящие данные являются корректным файлом изображения. Он расширяет [file](#yiivalidatorsfilevalidatorfile-)
...
Этот валидатор проверяет, являются ли входящие значения числовыми. Он эквивалентен валидатору [double](#yiivalidatorsnumbervalidatordouble-).
...
[безопасным атрибутом](structure-models.md#Безопасные-Атрибуты-).
