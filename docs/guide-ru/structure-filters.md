...
Фильтры — это объекты, которые могут запускаться как перед так и после [действий контроллера](structure-controllers.md#Действия-).
...
[использования поведений](concept-behaviors.md#Прикрепление-поведений-). Вы можете объявлять фильтры в классе контроллера
...
> Note: При объявлении фильтров в модулях или приложениях, следует использовать [маршруты](structure-controllers.md#Маршруты-)
...
[жизненном цикле](structure-applications.md#Жизненный-цикл-приложения-). По этой причине, ContentNegotiator разработан так, что
помимо фильтра может использоваться как [компонент предварительной загрузки](structure-applications.md#yiibaseapplicationbootstrapbootstrap-). Например,
вы можете настроить его в [конфигурации приложения](structure-applications.md#Конфигурации-приложения-):
