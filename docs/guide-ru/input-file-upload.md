...
В представлении, в вызов метода `fileInput()`, нужно добавить параметр `multiple` для того, чтобы поле *input* позволяло выбирать несколько файлов одновременно. Нужно изменить `imageFiles` на `imageFiles[]`, так чтобы атрибут передавался в виде массива:
