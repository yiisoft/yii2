...
[конфигурации приложения](structure-applications.md#Конфигурации-приложения-):
...
[[yii\base\Module::controllerMap]], аналогично тому, [как это делается в приложении](structure-applications.md#yiibaseapplicationcontrollermapcontrollermap-).
...
соответствует [идентификатору контроллера](structure-controllers.md#Маршруты-). Например, если контроллер реализуется
...
В модуле можно задать [шаблон](structure-views.md#Шаблоны-), который будет использоваться для рендеринга всех представлений
...
в конфигурации приложения. Следующий код в [конфигурации приложения](structure-applications.md#Конфигурации-приложения-)
...
Как маршруты приложения используются для обращения к контроллерам приложения, [маршруты](structure-controllers.md#Маршруты-)
модуля используются, чтобы обращаться к контроллерам этого модуля. Маршрут контроллера в модуле должен начинаться с
идентификатора модуля, за которым следуют [идентификатор контроллера](structure-controllers.md#id-контроллеров-) и
[идентификатор действия](structure-controllers.md#id-действий-). Например, если в приложении задействован модуль `forum`,
...
Зачастую внутри модуля может потребоваться доступ к экземпляру [класса модуля](#Классы-модулей-), через который получаются
