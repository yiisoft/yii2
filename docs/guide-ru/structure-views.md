...
Кроме `$this`, в виде могут быть доступны другие переменные, такие как `$form` и `$model` из примера выше. Эти переменные представляют собой данные, которые передаются в вид [контроллерами](structure-controllers.md) или другими объектами, которые вызывают [рендеринг вида](#Рендеринг-видов-).
...
* [[yii\base\Controller::render()|render()]]: рендерит [именованный вид](#Именованные-виды-) и применяет [шаблон](#Шаблоны-)
  к результату рендеринга.
* [[yii\base\Controller::renderPartial()|renderPartial()]]: рендерит [именованный вид](#Именованные-виды-) без шаблона.
* [[yii\web\Controller::renderAjax()|renderAjax()]]: рендерит [именованный вид](#Именованные-виды-) без шаблона,
...
* [[yii\base\Widget::render()|render()]]: рендерит [именованный вид](#Именованные-виды-).
...
* [[yii\base\View::render()|render()]]: рендерит [именованный вид](#Именованные-виды-).
* [[yii\web\View::renderAjax()|renderAjax()]]: рендерит [именованный вид](#Именованные-виды-) и добавляет зарегистрированные JS/CSS скрипты и стили. Обычно используется для рендеринга результата AJAX запроса.
...
Затем, в [шаблоне](#Шаблоны-), который также является видом, вы можете отобразить хлебные крошки используя данные, переданные через [[yii\base\View::params|params]].
...
Если вы хотите получить доступ к другим данным из шаблона, используйте метод явной передачи (он описан в секции [Доступ к данным в видах](#Доступ-к-данным-из-видов-) настоящего документа). Если вы хотите
передать данные из вида шаблону, вы можете использовать метод, описанный в [передаче данных между видами](#Передача-данных-между-видами-).


### Использование шаблонов <span id="using-layouts"></span>

Как было описано в секции [Рендеринг в контроллерах](#Рендеринг-в-контроллерах-), когда вы рендерите вид, вызывая метод [[yii\base\Controller::render()|render()]] из контроллера, к результату рендеринга будет применен шаблон. По умолчанию будет использован шаблон `@app/views/layouts/main.php` .
...
сам компонент `view` приложения. Вы можете сконфигурировать компонент в [конфигурации приложения](structure-applications.md#Конфигурации-приложения-) таким образом:
...
У каждой страницы должен быть заголовок. Обычно заголовок выводится в [шаблоне](#Шаблоны-). Однако на практике
...
Как и [мета тэги](#Регистрация-мета-тэгов-), link тэги полезны во многих случаях, как, например, задание уникальной favicon, указание на RSS фид или указание OpenID сервера для авторизации. С link тэгами можно работать аналогично работе с мета тэгами, вызывая метод [[yii\web\View::registerLinkTag()]]. Например,
...
приведет к многократному повторению похожего кода. Вместо этого вы можете использовать [отдельное действие](structure-controllers.md#Отдельные-действия-)  [[yii\web\ViewAction]] в контроллере. Например,
...
* используйте [шаблоны](#Шаблоны-) для отображения основных секций разметки сайта (верхняя часть (хедер), нижняя часть (футер) и т.п.)
