...
Если нужно сформировать ссылку `tel`, можно использовать следующий подход (актуально для мобильных устройств):

```php
<?= Html::tel('Позвоните там', '+71111111111') ?>
```

Если нужно сформировать ссылку `skype`, можно использовать следующий подход:

```php
<?= Html::skype('Позвоните нам в Skype', 'skype_account', 'call') ?>
```
