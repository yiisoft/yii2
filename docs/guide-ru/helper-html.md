...
С помощью хелперов `Html::ul()` и `Html::ol()` из массива можно сформировать маркированные и упорядоченные списоки соответственно.
Рассмотрим пример:
...
При генерации многомерных массивов элемент списка ребенка вкладывается в идущий выше список родителя. Если передать 
...
В результате на выходе от вышеприведённого примера получаем следующий html код
...
В данном примере мы использовали callback функцию для обработки каждого элемента списка, обратите внимание, так как
