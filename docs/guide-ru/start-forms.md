...
>Информация: В данном простом примере мы выдаем диалог подтверждения, тем не менее, когда вы обрабатываете POST данные, вам
необходимо использовать refresh или redirect для того, чтобы избежать [повторной](http://en.wikipedia.org/wiki/Post/Redirect/Get) отправки формы.
