...
Однако если ваше Yii-приложение  работает за обратным прокси-сервером, вам нужно добавить дополнительную конфигурацию для получения этой информации, поскольку прямой клиент теперь является прокси-сервером, а IP-адрес пользователя передается Yii-приложению с помощью заголовка, установленного прокси-сервером.

Вы не должны слепо доверять заголовкам, предоставленным прокси, если вы явно не доверяете прокси.
Начиная с 2.0.13, Yii поддерживает настройку доверенных прокси через
...
свойства компонента `request`.

Ниже приведена конфигурация компонента `request` для приложения, которое работает за массивом обратных прокси, которые расположены в IP-сети `10.0.2.0/24`:
