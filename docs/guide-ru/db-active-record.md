...
2. Настроить объект запроса вызовом [методов построения запросов](db-query-builder.md#Построение-запросов-);
3. Вызвать один из [методов получения данных](db-query-builder.md#Методы-выборки-) для извлечения данных в виде объектов
Active Record.
...
- ассоциативный массив: ключи массива интерпретируются как названия столбцов, а значения - как содержимое столбцов,
  которое следует искать. За подробностями вы можете обратиться к разделу [Hash Format](db-query-builder.md#Формат-массива-)
...
> Tip: Вышеприведённый пример демонстрирует общий способ преобразования данных в различные форматы. Если вы
  работаете с датами и временем, вы можете использовать [DateValidator](tutorial-core-validators.md#yiivalidatorsdatevalidatordate-) и
...
[возможностью массового присваивания](structure-models.md#Массовое-Присвоение-). Как будет показано ниже, используя эту
возможность, вы можете одним PHP выражением присвоить значения множества атрибутов Active Record объекту. Запомните
однако, что только [безопасные атрибуты](structure-models.md#Безопасные-Атрибуты-) могут быть массово присвоены.
...
> Note: Сравнение старых и новых значений будет осуществлено с помощью оператора `===`, так что значение будет
  ...
  [фильтрацию данных](input-validation.md#Фильтрация-данных-): `['attributeName', 'filter', 'filter' => 'intval']`.
...
Есть два способа использования [транзакций](db-dao.md#Исполнение-транзакций-) при работе с Active Record.
...
[сценариев](structure-models.md#Сценарии-), а значения соответствуют операциям, которые должны быть выполнены с помощью
