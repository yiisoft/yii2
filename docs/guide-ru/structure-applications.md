...
[конфигурационных файлов](concept-configurations.md#Конфигурационные-файлы-), например, `web.php` - файл в приведённом выше примере.
...
- ID, указанный в [компонентах](#yiibaseapplicationcomponentscomponents-);
- ID модуля, указанный в [модулях](#yiibaseapplicationmodulesmodules-);
- название класса;
- массив конфигурации;
...
[компоненты приложения](#Приложения), которые Вы можете использовать в других местах.
Например,
...
По-умолчанию, Yii задает соответствие между ID контроллера и его классом согласно данному [соглашению](#yiibaseapplicationcontrollernamespacecontrollernamespace-)
...
В случае, если вы хотите переопределить соответствия как описано выше, вы можете настроить свойство [controllerMap](#yiibaseapplicationcontrollermapcontrollermap-).
...
Аналогично свойству [language](#yiibaseapplicationlanguagelanguage-), вы должны указать данное свойство в рамках стандарта [IETF](http://en.wikipedia.org/wiki/IETF_language_tag).
Например, для английского языка используется `en`, в то время как для английского в США - `en-US`.
...
Значение по умолчанию равно `'main'`, означающее, что должен быть использован шаблон `main.php` в [папке шаблонов](#yiibaseapplicationlayoutpathlayoutpath-).
Если оба свойства [папка шаблонов](#yiibaseapplicationlayoutpathlayoutpath-) и [папка представлений](#yiibaseapplicationviewpathviewpath-) имеют значение по умолчанию,
...
в [папке представлений](#yiibaseapplicationviewpathviewpath-). Если значение [папки представлений](#yiibaseapplicationviewpathviewpath-) является значением по умолчанию, то
...
Использование синтаксиса `on eventName` детально описано в разделе [Конфигурации](concept-configurations.md#Формат-конфигурации-).
