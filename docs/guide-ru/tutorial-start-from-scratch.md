...
Затем необходимо подождать, чтобы репозитарии загрузился на ваш компьютер. Поскольку измененный шаблон не должен быть "запушен"(push) обратно, вы можете удалить из загрузки `.git` директорию со всем её содержимым.
