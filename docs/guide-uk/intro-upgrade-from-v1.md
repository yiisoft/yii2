...
представлення, навіть до представлення, яке сформоване за межами контексту контролера або віджету. Також, більше 
не існує компонента `CThemeManager`. Замість цього є властивість `theme` компонента додатка `view`, яку можна сконфігурувати.
