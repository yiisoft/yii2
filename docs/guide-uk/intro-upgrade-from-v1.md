...
Для створення часткових представлень тепер використовується метод `$this->render()`, а не `$this->renderPartial()`.
...
представлення, навіть до представлення, яке створене за межами контексту контролера або віджету. Також, більше 
не існує компонента `CThemeManager`. Замість цього є властивість `theme` компонента додатка `view`, яку можна  конфігурувати.
...
У Yii 2.0 були прибрані вбудовані форматери часу та чисел на користь 
...
Фільтри дій тепер зроблені за допомогою поведінок. При визаченні нового фільтру його необхідно успадкувати від [[yii\base\ActionFilter]].
...
У Yii 2.0 внесено безліч змін в роботу [Active Record](db-active-record.md). Основними двома являються
