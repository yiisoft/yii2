...
class QueryBuilder extends \yii\db\QueryBuilder
{
    ...
    {
        return $this->insertWithOutput($table, $columns, $params);
    }
     ...
     * @param array $outputColumns columns added to the OUTPUT part of the query
     * @return string the INSERT SQL
     */
    public function insertWithOutput($table, $columns, &$params, $outputColumns = [])
    {
        ...
        if (empty($columns) && !empty($outputColumns)) {
            $columns = array_fill_keys($outputColumns, new Expression('DEFAULT'));
        }
        $names = [];
        $placeholders = [];
        ...
        $output = [];
        foreach ((array)$outputColumns as $name) {
            $output[] = 'INSERTED.' . $schema->quoteColumnName($name);
        }

        return 'INSERT INTO ' . $schema->quoteTableName($table)
        . (!empty($names) ? ' (' . implode(', ', $names) . ')' : '')
        . (!empty($output) ? ' OUTPUT ' . implode(', ', $output) : '')
