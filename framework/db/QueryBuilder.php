...
class QueryBuilder extends \yii\base\BaseObject
{
     ...
     * ```
     *
     * Each row may contain different columns when `$columns` is empty. Missing columns are left unchanged.
     *
     ...
     * and mapped to column names (same convention as [[batchInsert()]]).
     ...
     * appended to the key-based condition.
     * @param array $params the binding parameters that will be generated by this method.
     * They should be bound to the DB command later.
    ...
    public function batchUpdate($table, $rows, $columns = [], $keys = [], $condition = '', &$params = [])
    {
        ...
        foreach ($keys as $keyCol) {
            $quotedKeys[$keyCol] = $this->db->quoteColumnName($keyCol);
        }
        ...
        if ($condition !== '' && $condition !== []) {
            ...
            if ($conditionSql !== '') {
                $whereSql = '(' . $whereSql . ') AND (' . $conditionSql . ')';
            }
        }
