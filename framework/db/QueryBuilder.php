...
class QueryBuilder extends \yii\base\BaseObject
{
     ...
     *
     * ```
     * $sql = $queryBuilder->batchUpdate('user', [
     *     ['id' => 1, 'name' => 'Tom', 'age' => 30],
     *     ['id' => 2, 'name' => 'Jane'],
     * ], 'id', $params);
     * ```
     *
     * Each row may contain different columns. Missing columns are left unchanged.
     *
    ...
    public function batchUpdate($table, $rows, $key, &$params = [])
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($keyValue === null) {
                $hasNullKey = true;
                $whenCondition = $quotedKey . ' IS NULL';
            } else {
                $keyPlaceholder = $this->bindParam($keyValue, $params);
                $keyPlaceholders[] = $keyPlaceholder;
                $whenCondition = $quotedKey . '=' . $keyPlaceholder;
            }
        ...
        if ($hasNullKey) {
            if (!empty($keyPlaceholders)) {
                return $sql . ' WHERE (' . $quotedKey . ' IN (' . implode(', ', $keyPlaceholders) . ') OR ' . $quotedKey . ' IS NULL)';
            }
            return $sql . ' WHERE ' . $quotedKey . ' IS NULL';
        }

        return $sql . ' WHERE ' . $quotedKey . ' IN (' . implode(', ', $keyPlaceholders) . ')';
    }
