...
* `yii\BaseYii::powered()` method has been removed. Please add "Powered by Yii" link either right into HTML or using
  `yii\helpers\Html::a()`.
* `yii\db\BaseActiveRecord::bindModels()` used by `yii\db\BaseActiveRecord::link()` no longer saves the foreign record.
  You need to call `save()`, `insert()` or `update()` (depending on your needs) explicitly after linking, e. g.:
  ```php
  ...
  $order->save(false); // This was previously done by link()
  ```
...
* When hash format condition (array) is used in `yii\db\ActiveRecord::findOne()` and `findAll()`, the array keys (column names)
  ...
  records by different condition than a primary key search and violate your application business logic. So passing user input directly
  like this can cause problems and should be avoided.
