...
<head>
	...
	<title><?php
		if ($exception instanceof \yii\web\HttpException) {
			echo (int) $exception->statusCode . ' ' . Html::encode($exception->getName());
		} elseif ($exception instanceof \yii\base\Exception) {
			echo Html::encode($exception->getName() . ' – ' . get_class($exception));
		} else {
			echo Html::encode(get_class($exception));
		}
	?></title>
...
<body>
	<div class="header">
		<?php if ($exception instanceof \yii\base\ErrorException): ?>
			...
			<h1>
				<span><?php echo Html::encode($exception->getName()); ?></span>
		...
		<?php else: ?>
			...
			<h1><?php
				if ($exception instanceof \yii\web\HttpException) {
					echo '<span>' . $this->createHttpStatusLink($exception->statusCode, Html::encode($exception->getName())) . '</span>';
					echo ' &ndash; ' . $this->addTypeLinks(get_class($exception));
				} elseif ($exception instanceof \yii\base\Exception) {
					echo '<span>' . Html::encode($exception->getName()) . '</span>';
					echo ' &ndash; ' . $this->addTypeLinks(get_class($exception));
				} else {
					echo '<span>' . Html::encode(get_class($exception)) . '</span>';
				}
			?></h1>
		<?php endif; ?>
		<h2><?php echo Html::encode($exception->getMessage()); ?></h2>
