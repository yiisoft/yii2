 ...
 *
 * 被比较的值可以是其他的属性的值（通过 [[compareAttribute]] 字段设置），也可以是一个常量（ 通过 [[compareValue]] 设置）。
 * 如果两个都设置了，后者优先。如果都没有设置，这个属性将会跟其他以当前属性名为前缀，以 "_repeat" 为后缀的属性进行比较。
 *
 * CompareValidator 支持不同的比较运算符，通过 [[operator]] 属性指定。
 *
 * 默认的比较功能是基于字符串值，即待比较的值将以字节的形式进行比较。当比较数值型时，确认已将 [[$type]] 属性的值设置为 [[TYPE_NUMBER]]
 * 以启用数值比较。
 *
...
class CompareValidator extends Validator
{
     ...
     * @var string 待比较的属性值。当它和 [[compareValue]] 都被设置时，后者优先。如果都没有设置，它会跟以当前属性名后缀 "_repeat" 的属性进行比较。
     * 例如：如果当前比较的是 'password' 属性，那么它比较的属性将为 'password_repeat'
     * @see compareValue
     */
     ...
     * @var mixed 待比较的常量值。当这个属性和 [[compareAttribute]] 同时设置时，这个属性优先。
     ...
     * @var string 用户定义错误消息。它可包含如下的占位符，并将被校验器自动的替换：
     *
     * - `{attribute}`: 待校验的属性标签名
     * - `{value}`: 待校验的属性值
