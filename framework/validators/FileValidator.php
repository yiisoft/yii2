...
class FileValidator extends Validator
{
     ...
     * 它可以是一个数组，或者一个由空格或者逗号分隔的字符串（例如："gif, jpg"）。
     * 扩展名称大小写不敏感，默认为 null ，代表允许所有的文件扩展名。
     ...
     * @var bool 是否检查文件类型（扩展名）和 mime-type 是否符合。如果 mime-type 得到的扩展名和文件名称扩展名不一致，文件会被认为非法。
     */
     ...
     * 它可以是一个 MIME 类型字符串数组，或者一个由空格或者逗号分隔的字符串（例如："text/plain, image/png"）。
     ...
     * 默认为1，代表上传单文件。你可以设置一个大一点的值，这样允许同时上传多个文件。设置为 `0` 意味着同时上传的文件数目没有限制。
     *
     ...
     * @var int 指定的属性最少持有的文件数量。默认为0，代表无限制。大一点的值意味着最少那么多的文件应该被上传。
     *
     ...
     * 注意：这是校验错误消息。为了确保文件上传是必须的，你需要设置属性 [[skipOnEmpty]] 值为 `false`。
     */
