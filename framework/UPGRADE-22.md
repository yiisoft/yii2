...
`batchInsert()` into an `IDENTITY`-backed table raises `ORA-00001`: Oracle evaluates the `IDENTITY` default once per
source row of `INSERT ALL ... SELECT 1 FROM SYS.DUAL`, so every `INTO` branch receives the same auto-generated value.
Use sequential `Schema::insert()` calls or include explicit primary-key values per row.
