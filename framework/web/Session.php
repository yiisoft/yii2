 ...
 * [[destroySession()]], [[gcSession()]] and [[sessionIdExists()]].
 *
...
class Session extends Component implements \IteratorAggregate, \ArrayAccess, \Countable
{
     ...
     * @deprecated since 2.0.56. Override [[sessionIdExists()]] instead, so that PHP replaces an unknown session ID
     * during `session_start()`. The ID flagged here is still regenerated by [[open()]] for backward compatibility.
     */
     ...
     * [[readSession()]], [[writeSession()]], [[destroySession()]], [[gcSession()]] and [[sessionIdExists()]].
    ...
    public function open()
    {
        ...
        // skip the deprecated regeneration when PHP already replaced the flagged ID through sessionIdExists()
        if (
            $this->getUseStrictMode()
            && $this->_forceRegenerateId
            && $this->_forceRegenerateId === $this->getId()
        ) {
            $this->regenerateID();
     ...
     * PHP calls this method through [[SessionHandler::validateId()]] only when [[useStrictMode]] is enabled:
     * `session_start()` replaces an ID reported as missing with [[createSessionId()]], and `session_regenerate_id()`
     * retries a new ID reported as existing.
     * The default implementation accepts every ID. Override it if [[useCustomStorage]] returns `true`, so that strict
     * mode rejects session IDs not present in the storage.
     * @internal Do not call this method directly.
     * @param string $id session ID
     * @return bool whether a session with the given ID exists in the storage
     * @since 2.0.56
     */
