...
class UrlManager extends Component
{
    ...
    public function getScriptUrl()
    {
        if ($this->_scriptUrl === null) {
            $this->_scriptUrl = $this->getRequest()->getScriptUrl();
        }
    ...
    public function getHostInfo()
    {
        if ($this->_hostInfo === null) {
            $this->_hostInfo = $this->getRequest()->getHostInfo();
        }
    ...
    protected function getRequest()
    {
        ...
        if (!$request instanceof Request) {
            throw new InvalidConfigException('`Yii::$app->request` should be an instance of `' . Request::class . '`.');
        }
    ...
    /**
     * @return Controller the currently active controller instance
     * @throws InvalidConfigException
     * @since 2.1.0
     */
    protected function getController()
    {
        ...
        if (!$controller instanceof Controller) {
            throw new InvalidConfigException('`Yii::$app->controller` should be an instance of `' . Controller::class . '`.');
        }
    ...
    public function createUrlToRoute($route, $scheme = false)
    {
        ...
        if ($scheme !== false) {
            return $this->createAbsoluteUrl($route, is_string($scheme) ? $scheme : null);
        }
    ...
    protected function normalizeRoute($route)
    {
        ...
        if (strpos($route, '/') === false) {
            ...
            return $route === '' ? $this->getController()->getRoute() : $this->getController()->getUniqueId() . '/' . $route;
        }

        // relative to module
        return ltrim($this->getController()->module->getUniqueId() . '/' . $route, '/');
    }
     ...
     * [[getHostInfo()]]) will be returned. If `$url` is already an absolute URL, its scheme
     ...
     *
     * // /images/logo.gif
     * echo $urlManager->to('@web/images/logo.gif');
     *
     * // images/logo.gif
     * echo $urlManager->to('images/logo.gif');
     *
     * // http://www.example.com/images/logo.gif
     * echo $urlManager->to('@web/images/logo.gif', true);
     *
     * // https://www.example.com/images/logo.gif
     * echo $urlManager->to('@web/images/logo.gif', 'https');
     *
    ...
    public function createUrlTo($url = '', $scheme = false)
    {
        ...
        if (Url::isRelative($url)) {
            ...
            $url = $this->getHostInfo() . '/' . ltrim($url, '/');
        }
     ...
     * @since 2.1.0
     */
    protected function ensureScheme($url, $scheme)
    {
        if (Url::isRelative($url) || !is_string($scheme)) {
            return $url;
        }
