...
class MigrateController extends BaseMigrateController
{
    ...
    protected function generateMigrationSourceCode($params)
    {
        ...
        $table = null;
        if (preg_match('/^create_junction(?:_table_for_|_)(.+)_and_(.+)(?:_tables?)?$/U', $name, $matches)) {
        ...
        } elseif (preg_match('/^add_(.+)(?:_column)?_to_(.+)(?:_table)?$/U', $name, $matches)) {
            $templateFile = $this->generatorTemplateFiles['add_column'];
            $table = mb_strtolower($matches[2], Yii::$app->charset);
        } elseif (preg_match('/^drop_(.+)(?:_column)?_from_(.+)(?:_table)?$/U', $name, $matches)) {
            $templateFile = $this->generatorTemplateFiles['drop_column'];
            $table = mb_strtolower($matches[2], Yii::$app->charset);
        } elseif (preg_match('/^create_(.+)(?:_table)?$/U', $name, $matches)) {
            $this->addDefaultPrimaryKey($fields);
            $templateFile = $this->generatorTemplateFiles['create_table'];
            $table = mb_strtolower($matches[1], Yii::$app->charset);
        } elseif (preg_match('/^drop_(.+)(?:_table)?$/U', $name, $matches)) {
            $this->addDefaultPrimaryKey($fields);
