...
- Bug #7670: Added `yii\wew\UrlNormalizer` for normalizing requests with and without trailing slashes (rob006, cronfy, klimov-paul)
- Bug #9101: Fixed `yii\web\View` to respect `yii\web\AssetManager::$appendTimestamp` property (githubjeka, silverfire)
...
- Bug #6266: Clicking on reset button does not hide error summary when using `yii\widgets\ActiveForm` (InteLigent, qiangxue)
...
- Bug #4905: `yii\widgets\ActiveForm::$validationDelay` doesn't delay after `keyrelease` when `$validateOnType=true` (qiangxue)
