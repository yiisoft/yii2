...
- Enh #7263, #8743: Added a new syntax for resolving table aliases to `yii\db\Query`, `{{@tablename}}` will now be replaced by the table alias (cebe)
...
- Enh #9326: Added methods `getAlias()` and `applyAlias()` to `yii\db\Query` for referring to already defined table aliases when modifying queries (cebe)
