...
BEGIN EXECUTE IMMEDIATE 'DROP TABLE "legacy_identity_via_trigger"'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;--

BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE "legacy_identity_via_trigger_SEQ"'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END;--
...
/**
 * Legacy fixture: a deliberate sequence + BEFORE INSERT trigger pair.
 *
 * Exists only to exercise the `Schema::getTableSequenceName()` legacy fallback
 * branch (USER_DEPENDENCIES + USER_TRIGGERS) for users with pre-22.0 Oracle
 * schemas. New tables in this fixture use native IDENTITY columns instead.
 */
CREATE TABLE "legacy_identity_via_trigger" (
  "id" integer NOT NULL,
  "name" varchar2(64) NOT NULL,
  CONSTRAINT "legacy_identity_via_trigger_PK" PRIMARY KEY ("id") ENABLE
);

CREATE SEQUENCE "legacy_identity_via_trigger_SEQ";

/* TRIGGERS */

CREATE TRIGGER "legacy_identity_via_trigger_TRG" BEFORE INSERT ON "legacy_identity_via_trigger" FOR EACH ROW BEGIN <<COLUMN_SEQUENCES>> BEGIN
  IF INSERTING AND :NEW."id" IS NULL THEN SELECT "legacy_identity_via_trigger_SEQ".NEXTVAL INTO :NEW."id" FROM SYS.DUAL; END IF;
