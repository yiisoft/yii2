...
final class QueryBuilderTest extends BaseQueryBuilder
{
    ...
    public static function conditionProvider(): array
    {
        return array_merge(
            ...
            [
                ...
                [['@>', 'id', new ArrayExpression(1)], '"id" @> ARRAY[]', []],
                [['@>', 'id', new ArrayExpression(false)], '"id" @> ARRAY[]', []],
                ...
                [['@>', 'id', new ArrayExpression([[1,2], [3,4]], 'float', 2)], '"id" @> ARRAY[ARRAY[:qp0, :qp1]::float[], ARRAY[:qp2, :qp3]::float[]\\]::float[][]', [':qp0' => 1, ':qp1' => 2, ':qp2' => 3, ':qp3' => 4]],
    ...
    public function testResetSequenceForGeneratedAsIdentityColumn(): void
    {
        $qb = $this->getQueryBuilder();
        ...
        self::assertEquals(
            $expected,
            $sql,
            "Reset sequence SQL on identity column without explicit value must use 'SETVAL' with 'COALESCE(MAX(id),0)+1'.",
        );
        ...
        self::assertEquals(
            $expected,
            $sql,
            "Reset sequence SQL on identity column with explicit value must use 'SETVAL' with the literal seed.",
        );
    }
