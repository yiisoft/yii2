...
class GridViewTest extends TestCase
{
    ...
    {
        return new GridView(
            array_merge(['dataProvider' => new ArrayDataProvider(['allModels' => []])], $config),
        );
    }
    ...
    {
        /** @var GridView $widget */
        // `GridView::run()` also calls `parent::run()` which renders the grid; suppress the HTML so the test output
        // stays clean.
        ob_start();
