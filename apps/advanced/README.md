...
If you already have a server configured or your application is not located in a folder called `advanced`, you may need to adjust the `test_entry_url` in `backend/codeception.yml`, `common/codeception.yml`, `console/codeception.yml` and `frontend/codeception.yml`.
...
If you have run `../vendor/bin/codecept build` for each application, you can run all tests by one command: `vendor/bin/codecept run`
